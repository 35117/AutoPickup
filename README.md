...
- 条件拾取：物品耐久低于设定值不拾取
- 条件拾取：弹夹子弹数低于设定值不拾取
- 隔墙拾取开关：默认关闭（被墙/结构/大型物体遮挡的掉物不会自动拾取），开启则无视遮挡
- 丢弃冷却：扔出物品后一段时间内不自动拾取该物品（防止刚扔的立刻捡回，时长可配置）
...
| Pickup | DropCooldownSeconds | 2 | 扔出物品后多少秒内不自动拾取该物品（0 关闭） |
| Pickup | PickupThroughWalls | false | 隔墙拾取：false=被墙/结构遮挡的掉物不拾取（需视线），true=无视遮挡 |
| Pickup | MinMagazineAmmo | 0 | 条件拾取：弹夹子弹数低于此值不拾取（0 不限制） |
...
- 想腾背包扔东西：扔出后默认 2 秒内不会被吸回，可调 DropCooldownSeconds（0 关闭）
- 不想隔墙捡东西：保持 PickupThroughWalls 为 false，墙后/大型物体后的掉物不会自动拾取
