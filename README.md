...
版本号格式为 `年.月.日.第几版`，例如 `26.8.12.1` 表示 2026 年 8 月 12 日当天上传的第 1 版。
...
- 背包中按住 Alt 单击物品：将该物品加入/移出黑名单（原版背包无悬停物品 API，采用与自动合成插件一致的 Alt+点击方案）
...
| Shortcuts | AltClickAddBlacklist | true | 背包中 Alt+单击切换黑名单 |

## 使用技巧

- 想要某物品：准星对着掉物按 Alt+F，加入白名单（再按一次移出）
- 不想要某物品：打开背包，按住 Alt 点击该物品，加入黑名单（再点一次移出）
